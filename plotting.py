...
def plotRoc(fpr, tpr, auc, labels, linestyle, name, legend=True):
    ...
    plt.figtext(0.25, 0.90, name, fontweight='bold', wrap=True, horizontalalignment='right', fontsize=14)
...
def makeRoc(y, predict_test, labels, linestyle='-', name='hls4ml', legend=True ):
    if 'j_index' in labels:
        labels.remove('j_index')

    fpr, tpr, auc1 = rocData(y, predict_test, labels)
    plotRoc(fpr, tpr, auc1, labels, linestyle, name, legend=legend)
    return predict_test
